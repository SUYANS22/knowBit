...
bool runLoginGUI() {
    ...
    while (!WindowShouldClose()) {
        ...
        if (!isLoggedIn) {
            ...
            DrawText("know", 105, 35, 30, WHITE);
            DrawText("Bit", 105 + knowWidth + 5, 35, 30, saffronOrange); 
            ...
            Rectangle mntDest   = { 15, 95, 555, 360 }; 
            DrawTexturePro(mountain, mntSource, mntDest, (Vector2){0, 0}, 0.0f, WHITE);
            DrawTextureEx(textLogo, (Vector2){ 40, 475 }, 0.0f, 0.42f, WHITE); 
            DrawText("Plan. Prioritize. Get It Done.", 50, 590, 30, WHITE); 
            DrawRectangleRoundedLinesEx(formBox, 0.05f, 10, 1.5f, saffronOrange);
            DrawText("Welcome Back!", 750, 130, 30, saffronOrange);
            DrawTextureEx(hand, (Vector2){ 970, 125 }, 0.0f, 0.15f, WHITE); 
            DrawText("Log in to Continue Your Journey", 650, 180, 18, tealText);
            DrawText("Username", 630, 230, 18, tealText);
            ...
            DrawTextureEx(userIcon, (Vector2){ 645, 271 }, 0.0f, 0.12f, WHITE);
            DrawText(usernameInput, 690, 272, 20, WHITE);
            DrawTextureEx(eyeIcon, (Vector2){ 1045, 272 }, 0.0f, 0.04f, WHITE);
            if (mouseOnUser && ((int)(GetTime() * 2) % 2 == 0)) {
                DrawText("_", 690 + MeasureText(usernameInput, 20), 272, 20, saffronOrange);
            }
            DrawText("Password", 630, 330, 18, tealText);
            ...
            DrawText(hiddenPass.c_str(), 690, 375, 20, WHITE);
            if (mouseOnPass && ((int)(GetTime() * 2) % 2 == 0)) {
                DrawText("_", 690 + MeasureText(hiddenPass.c_str(), 20), 375, 20, saffronOrange);
            }
            DrawText("  Remember Me", 630, 430, 15, WHITE);
            DrawText("Forgot Password? ", 920, 430, 15, saffronOrange);
            ...
            DrawRectangleRoundedLinesEx(guestBtn, 0.2f, 10, 1.5f, tealText);
            DrawText(" Guest Mode", 900, 502, 20, tealText);
        ...
        //  else {
        //     DrawText(TextFormat("    Welcome to knowBit, %s!", usernameInput), 380, 300, 40, saffronOrange);
        //     DrawText("We are still in the process of making the dashboard...", 430, 370, 20, WHITE);
        //     DrawText("     Press ESC to exit.", 510, 420, 20, textMuted);
        // }
